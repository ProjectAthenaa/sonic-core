...
func (tq *TaskQuery) QueryProduct() *ProductQuery {
...
func (tq *TaskQuery) QueryProxyList() *ProxyListQuery {
	query := &ProxyListQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryTaskGroup chains the current query on the "TaskGroup" edge.
func (tq *TaskQuery) QueryTaskGroup() *TaskGroupQuery {
	query := &TaskGroupQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TaskQuery) WithProduct(opts ...func(*ProductQuery)) *TaskQuery {
	query := &ProductQuery{config: tq.config}
...
func (tq *TaskQuery) WithProxyList(opts ...func(*ProxyListQuery)) *TaskQuery {
	query := &ProxyListQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withProxyList = query
	return tq
}
...
// WithTaskGroup tells the query-builder to eager-load the nodes that are connected to
// the "TaskGroup" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TaskQuery) WithTaskGroup(opts ...func(*TaskGroupQuery)) *TaskQuery {
	query := &TaskGroupQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withTaskGroup = query
	return tq
}
...
func (tq *TaskQuery) sqlAll(ctx context.Context) ([]*Task, error) {
	...
	if query := tq.withProduct; query != nil {
		...
		if err := sqlgraph.QueryEdges(ctx, tq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "Product": %w`, err)
		}
	...
	if query := tq.withProxyList; query != nil {
		...
		if err := sqlgraph.QueryEdges(ctx, tq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "ProxyList": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "ProxyList" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.ProxyList = append(nodes[i].Edges.ProxyList, n)
			}
		}
	}
	...
	if query := tq.withProfileGroup; query != nil {
		...
		if err := sqlgraph.QueryEdges(ctx, tq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "ProfileGroup": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "ProfileGroup" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.ProfileGroup = append(nodes[i].Edges.ProfileGroup, n)
			}
		}
	}
