...
func (pq *ProfileQuery) QueryProfileGroup() *ProfileGroupQuery {
	query := &ProfileGroupQuery{config: pq.config}
...
func (pq *ProfileQuery) QueryShipping() *ShippingQuery {
	query := &ShippingQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProfileQuery) QueryBilling() *BillingQuery {
	query := &BillingQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProfileQuery) Clone() *ProfileQuery {
	...
	return &ProfileQuery{
		...
		withProfileGroup: pq.withProfileGroup.Clone(),
		withShipping:     pq.withShipping.Clone(),
		withBilling:      pq.withBilling.Clone(),
...
func (pq *ProfileQuery) WithProfileGroup(opts ...func(*ProfileGroupQuery)) *ProfileQuery {
	query := &ProfileGroupQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withProfileGroup = query
	return pq
}

// WithShipping tells the query-builder to eager-load the nodes that are connected to
// the "Shipping" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProfileQuery) WithShipping(opts ...func(*ShippingQuery)) *ProfileQuery {
	query := &ShippingQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withShipping = query
	return pq
}
...
func (pq *ProfileQuery) WithBilling(opts ...func(*BillingQuery)) *ProfileQuery {
	query := &BillingQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withBilling = query
	return pq
}
...
func (pq *ProfileQuery) sqlAll(ctx context.Context) ([]*Profile, error) {
	var (
		...
		_spec       = pq.querySpec()
		loadedTypes = [3]bool{
			pq.withProfileGroup != nil,
			pq.withShipping != nil,
			pq.withBilling != nil,
		}
	)
	if pq.withProfileGroup != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withProfileGroup; query != nil {
		ids := make([]uuid.UUID, 0, len(nodes))
		nodeids := make(map[uuid.UUID][]*Profile)
		for i := range nodes {
			if nodes[i].profile_group_profiles == nil {
				continue
			}
			fk := *nodes[i].profile_group_profiles
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "profile_group_profiles" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := pq.withShipping; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*Profile)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Shipping = []*Shipping{}
		}
		query.withFKs = true
		query.Where(predicate.Shipping(func(s *sql.Selector) {
			s.Where(sql.InValues(profile.ShippingColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.profile_shipping
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "profile_shipping" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "profile_shipping" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Shipping = append(node.Edges.Shipping, n)
		}
	}

	if query := pq.withBilling; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[uuid.UUID]*Profile, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Billing = []*Billing{}
		}
		var (
			edgeids []uuid.UUID
		...
		if err := sqlgraph.QueryEdges(ctx, pq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "Billing": %w`, err)
		}
		query.Where(billing.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "Billing" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Billing = append(nodes[i].Edges.Billing, n)
			}
		}
	}
