...
func (sq *SettingsQuery) QueryApp() *AppQuery {
...
func (sq *SettingsQuery) WithApp(opts ...func(*AppQuery)) *SettingsQuery {
	query := &AppQuery{config: sq.config}
...
func (sq *SettingsQuery) sqlAll(ctx context.Context) ([]*Settings, error) {
	...
	if query := sq.withApp; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "app_settings" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.App = n
			}
...
func (sq *SettingsQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	selector := builder.Select(t1.Columns(settings.Columns...)...).From(t1)
	if sq.sql != nil {
		selector = sq.sql
		selector.Select(selector.Columns(settings.Columns...)...)
	}
...
func (ss *SettingsSelect) sqlQuery() sql.Querier {
	selector := ss.sql
