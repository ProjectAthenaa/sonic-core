...
func (lq *LicenseQuery) QueryUser() *UserQuery {
...
// QueryStripe chains the current query on the "Stripe" edge.
func (lq *LicenseQuery) QueryStripe() *StripeQuery {
	query := &StripeQuery{config: lq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := lq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (lq *LicenseQuery) Clone() *LicenseQuery {
	...
	return &LicenseQuery{
		...
		withStripe: lq.withStripe.Clone(),
...
func (lq *LicenseQuery) WithUser(opts ...func(*UserQuery)) *LicenseQuery {
	query := &UserQuery{config: lq.config}
...
// WithStripe tells the query-builder to eager-load the nodes that are connected to
// the "Stripe" edge. The optional arguments are used to configure the query builder of the edge.
func (lq *LicenseQuery) WithStripe(opts ...func(*StripeQuery)) *LicenseQuery {
	query := &StripeQuery{config: lq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (lq *LicenseQuery) sqlAll(ctx context.Context) ([]*License, error) {
	...
	if query := lq.withUser; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_license" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := lq.withStripe; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Stripe = []*Stripe{}
		}
		query.withFKs = true
		query.Where(predicate.Stripe(func(s *sql.Selector) {
			s.Where(sql.InValues(license.StripeColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.license_stripe
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "license_stripe" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "license_stripe" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Stripe = append(node.Edges.Stripe, n)
		}
	}
