...
func (aq *AddressQuery) QueryShippingAddress() *ShippingQuery {
	query := &ShippingQuery{config: aq.config}
...
func (aq *AddressQuery) QueryBillingAddress() *ShippingQuery {
	query := &ShippingQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (aq *AddressQuery) Clone() *AddressQuery {
	...
	return &AddressQuery{
		...
		withShippingAddress: aq.withShippingAddress.Clone(),
		withBillingAddress:  aq.withBillingAddress.Clone(),
...
func (aq *AddressQuery) WithShippingAddress(opts ...func(*ShippingQuery)) *AddressQuery {
	query := &ShippingQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withShippingAddress = query
	return aq
}
...
func (aq *AddressQuery) WithBillingAddress(opts ...func(*ShippingQuery)) *AddressQuery {
	query := &ShippingQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withBillingAddress = query
	return aq
}
...
func (aq *AddressQuery) sqlAll(ctx context.Context) ([]*Address, error) {
	...
	if aq.withShippingAddress != nil || aq.withBillingAddress != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := aq.withBillingAddress; query != nil {
		...
		nodeids := make(map[uuid.UUID][]*Address)
		for i := range nodes {
			if nodes[i].shipping_billing_address == nil {
				continue
			}
			fk := *nodes[i].shipping_billing_address
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "shipping_billing_address" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.BillingAddress = n
			}
		}
	}
