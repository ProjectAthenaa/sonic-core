...
func (uq *UserQuery) QueryLicense() *LicenseQuery {
	query := &LicenseQuery{config: uq.config}
...
func (uq *UserQuery) WithLicense(opts ...func(*LicenseQuery)) *UserQuery {
	query := &LicenseQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withLicense = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withLicense; query != nil {
		...
		query.Where(predicate.License(func(s *sql.Selector) {
			s.Where(sql.InValues(user.LicenseColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_license
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_license" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_license" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.License = n
		}
	}
	...
	if query := uq.withApp; query != nil {
		...
		query.Where(predicate.App(func(s *sql.Selector) {
			s.Where(sql.InValues(user.AppColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_app
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_app" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
