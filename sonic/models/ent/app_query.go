...
type AppQuery struct {
	...
	withProxyLists    *ProxyListQuery
	withProfileGroups *ProfileGroupQuery
	withTaskGroups    *TaskGroupQuery
...
func (aq *AppQuery) QuerySettings() *SettingsQuery {
	query := &SettingsQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryProxyLists chains the current query on the "ProxyLists" edge.
func (aq *AppQuery) QueryProxyLists() *ProxyListQuery {
	query := &ProxyListQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryProfileGroups chains the current query on the "ProfileGroups" edge.
func (aq *AppQuery) QueryProfileGroups() *ProfileGroupQuery {
	query := &ProfileGroupQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryTaskGroups chains the current query on the "TaskGroups" edge.
func (aq *AppQuery) QueryTaskGroups() *TaskGroupQuery {
	query := &TaskGroupQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryAccountGroups chains the current query on the "AccountGroups" edge.
func (aq *AppQuery) QueryAccountGroups() *AccountGroupQuery {
	query := &AccountGroupQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AppQuery) Clone() *AppQuery {
	...
	return &AppQuery{
		...
		withSettings:      aq.withSettings.Clone(),
		withProxyLists:    aq.withProxyLists.Clone(),
		withProfileGroups: aq.withProfileGroups.Clone(),
		withTaskGroups:    aq.withTaskGroups.Clone(),
		withAccountGroups: aq.withAccountGroups.Clone(),
...
func (aq *AppQuery) WithSettings(opts ...func(*SettingsQuery)) *AppQuery {
	query := &SettingsQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withSettings = query
	return aq
}

// WithProxyLists tells the query-builder to eager-load the nodes that are connected to
// the "ProxyLists" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AppQuery) WithProxyLists(opts ...func(*ProxyListQuery)) *AppQuery {
	query := &ProxyListQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withProxyLists = query
	return aq
}

// WithProfileGroups tells the query-builder to eager-load the nodes that are connected to
// the "ProfileGroups" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AppQuery) WithProfileGroups(opts ...func(*ProfileGroupQuery)) *AppQuery {
	query := &ProfileGroupQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withProfileGroups = query
	return aq
}

// WithTaskGroups tells the query-builder to eager-load the nodes that are connected to
// the "TaskGroups" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AppQuery) WithTaskGroups(opts ...func(*TaskGroupQuery)) *AppQuery {
	query := &TaskGroupQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withTaskGroups = query
	return aq
}

// WithAccountGroups tells the query-builder to eager-load the nodes that are connected to
// the "AccountGroups" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AppQuery) WithAccountGroups(opts ...func(*AccountGroupQuery)) *AppQuery {
	query := &AccountGroupQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withAccountGroups = query
	return aq
}
...
func (aq *AppQuery) sqlAll(ctx context.Context) ([]*App, error) {
	var (
		...
		loadedTypes = [6]bool{
			aq.withUser != nil,
			aq.withSettings != nil,
			aq.withProxyLists != nil,
			aq.withProfileGroups != nil,
			aq.withTaskGroups != nil,
			aq.withAccountGroups != nil,
		}
	)
	if aq.withUser != nil {
		withFKs = true
	...
	if query := aq.withUser; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_app" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := aq.withSettings; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Settings = []*Settings{}
		}
		query.withFKs = true
		query.Where(predicate.Settings(func(s *sql.Selector) {
			s.Where(sql.InValues(app.SettingsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.app_settings
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "app_settings" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "app_settings" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := aq.withProxyLists; query != nil {
		...
		if err := sqlgraph.QueryEdges(ctx, aq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "ProxyLists": %w`, err)
		}
		query.Where(proxylist.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "ProxyLists" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.ProxyLists = append(nodes[i].Edges.ProxyLists, n)
			}
		}
	}
	...
	if query := aq.withProfileGroups; query != nil {
		...
		if err := sqlgraph.QueryEdges(ctx, aq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "ProfileGroups": %w`, err)
		}
		query.Where(profilegroup.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "ProfileGroups" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.ProfileGroups = append(nodes[i].Edges.ProfileGroups, n)
			}
		}
	}
	...
	if query := aq.withTaskGroups; query != nil {
		...
		if err := sqlgraph.QueryEdges(ctx, aq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "TaskGroups": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "TaskGroups" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.TaskGroups = append(nodes[i].Edges.TaskGroups, n)
			}
		}
	}
	...
	if query := aq.withAccountGroups; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.AccountGroups = []*AccountGroup{}
		}
		query.withFKs = true
		query.Where(predicate.AccountGroup(func(s *sql.Selector) {
			s.Where(sql.InValues(app.AccountGroupsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.app_account_groups
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "app_account_groups" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "app_account_groups" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.AccountGroups = append(node.Edges.AccountGroups, n)
		}
	}
